...
const PROTOCOLS = {
  "Phase 1": [
    { icon: "🚫", text: "No indoor dining, flights, or large gatherings", type: "critical" },
    { icon: "🌿", text: "Outdoor visits only (1–2 friends max, distanced)", type: "caution" },
    { icon: "🏠", text: "No indoor visitors at home", type: "critical" },
    { icon: "😷", text: "Mask for all shared indoor spaces", type: "caution" },
    { icon: "🧪", text: "Same-day rapid test required for any social contact", type: "warning" },
    { icon: "⏳", text: "Wait 1 week post-ATG for any visits", type: "warning" },
  ],
  "Phase 2": [
    { icon: "🚫", text: "No flights or large indoor events (50+ people)", type: "critical" },
    { icon: "👥", text: "Small indoor visits OK (up to 4 people, ventilated)", type: "caution" },
    { icon: "🍽️", text: "Outdoor dining OK; indoor dining with caution", type: "caution" },
    { icon: "😷", text: "Mask in crowded indoor spaces", type: "warning" },
    { icon: "🧪", text: "Rapid test if exposed to illness", type: "warning" },
  ],
  "Phase 3": [
    { icon: "✈️", text: "Flights OK with N95 mask", type: "caution" },
    { icon: "👥", text: "Indoor gatherings OK (moderate size)", type: "info" },
    { icon: "🍽️", text: "Indoor dining OK", type: "info" },
    { icon: "😷", text: "Mask during high community illness (optional otherwise)", type: "info" },
    { icon: "🧪", text: "Test if symptomatic or known exposure", type: "warning" },
  ],
  Clear: [
    { icon: "✅", text: "Resume normal social activities", type: "info" },
    { icon: "💊", text: "Continue all medications as prescribed", type: "warning" },
    { icon: "📋", text: "Maintain follow-up schedule with transplant team", type: "info" },
    { icon: "🧪", text: "Test if symptomatic", type: "info" },
  ],
};
...
function getSymptomStatus(temp, sys, dia, uo, co, gi, fat) {
  let a = 0;
  if (temp > 100.4 || temp < 96.0) a++;
  if (sys > 160 || sys < 90 || dia > 100 || dia < 50) a++;
...
.onboarding-card{background:var(--surface);border-radius:16px;padding:40px;max-width:440px;width:100%;box-shadow:var(--shadow-lg);border:1px solid var(--border)}
...
@media(max-width:768px){.grid{grid-template-columns:1fr}.app-container{padding:0 16px 80px}.header-title{font-size:18px}.phase-name{font-size:28px}.symptom-grid{grid-template-columns:1fr}.about-panel{padding:24px}.onboarding-card{padding:28px}.header{flex-wrap:wrap;gap:8px}.header-right{flex-wrap:wrap}}
`;
...
const DEFAULT_PATIENT = {
  alc: 0.5, weeksPostATG: 1,
  alcHistory: [{ date: "Week 0", alc: 0.1 },{ date: "Week 1", alc: 0.15 },{ date: "Week 2", alc: 0.22 },{ date: "Week 3", alc: 0.35 },{ date: "Week 4", alc: 0.42 },{ date: "Week 5", alc: 0.50 }],
  tacroTrough: 8, tacroDose: 4, mmfDose: 1000, predDose: 5,
  temp: 98.6, systolic: 120, diastolic: 80,
  urineOutput: "Normal", cough: "None", gi: "None", fatigue: "None",
  ...
  return (
    <div className="alc-chart" style={{ height: h + 30 }}>
      ...
      <div className="alc-chart-area">
        {thresholds.map(t=><div key={t.l} className="alc-threshold" style={{bottom:`${(t.v/max)*100}%`,borderColor:t.c,color:t.c}}><span style={{position:"absolute",right:0,top:-14,fontSize:10,opacity:.7}}>{t.l} ≥{t.v}</span></div>)}
        {history.map((pt,i)=>{
  ...
  return (
    <div className="about-overlay" onClick={onClose}>
      <div className="about-panel" onClick={e=>e.stopPropagation()}>
        ...
        <h3>The Problem</h3>
        <p>After a kidney transplant, patients receive intense immunosuppression therapy (often including ATG induction) that leaves their immune system severely compromised. Recovery follows a predictable trajectory tied to Absolute Lymphocyte Count (ALC) recovery. During this period, both patients and their household partners must follow evolving behavioral protocols — but this guidance is typically delivered as printed handouts at discharge, with no dynamic tool to track where you are or what the current rules should be.</p>
        <h3>Why I Built This</h3>
        <p>I received a kidney transplant and experienced this problem firsthand. My partner and I found ourselves constantly asking: "What phase are we in? Can we have people over yet? What should she do differently after a work event?" The answers change as ALC recovers, but there was no single tool that connected lab values to daily behavioral guidance — especially for the caregiver/partner, who has their own distinct protocol.</p>
        <h3>Key Product Decisions</h3>
        <p><strong>Two-view architecture:</strong> The patient and partner have fundamentally different needs and protocols. Rather than overloading one dashboard, I separated the views so each audience sees only what's relevant to them.</p>
        <p><strong>Linked accounts:</strong> Patient creates a profile and gets a share code. Their partner enters that code to link — mirroring a real multi-user health app without requiring a full auth system. Data persists across sessions.</p>
        <p><strong>Phase-driven protocols:</strong> Everything keys off the ALC-based recovery phase. As ALC rises, protocols automatically adjust — mirroring how transplant teams think about recovery milestones.</p>
        <p><strong>Infection vs. rejection balance:</strong> The immunosuppression monitor visualizes the core tension in transplant care — too much immunosuppression risks infection, too little risks rejection.</p>
        <p><strong>Partner exposure tracking:</strong> Partners' social activities directly impact patient safety. The daily exposure assessment gives partners a simple, actionable framework.</p>
        ...
        <p style={{marginBottom:4}}>
          <span className="about-tag">HIPAA-compliant hosting</span><span className="about-tag">EHR lab integration</span><span className="about-tag">Push notifications</span><span className="about-tag">Provider dashboard</span><span className="about-tag">Medication reminders</span><span className="about-tag">Multi-language support</span><span className="about-tag">Clinical validation study</span><span className="about-tag">Offline support</span>
        </p>
        <h3>Tech</h3>
...
function Onboarding({onComplete}) {
  ...
  async function createPatient() {
    ...
    const d = { ...DEFAULT_PATIENT, name: name.trim(), code: c, role: "patient", createdAt: new Date().toISOString() };
  ...
  async function linkPartner() {
    ...
    const pp = { role: "partner", name: name.trim() || "Partner", linkedCode: clean, linkedPatientName: pd.name, createdAt: new Date().toISOString() };
    ...
    <div className="onboarding">
      <div className="onboarding-card">
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
          <div className="header-logo">♥</div><span className="header-title">TransplantCare</span>
        </div>
        {step==="role"&&<>
          <h1>Welcome</h1>
          <p>This app helps kidney transplant patients and their partners track immunosuppression recovery and follow phase-appropriate behavioral protocols.</p>
          <div className={`role-option ${role==="patient"?"selected":""}`} onClick={()=>setRole("patient")}>
            <h3>🏥 I'm the Patient</h3><p>Track your ALC recovery, medications, symptoms, and see current protocols.</p>
          </div>
          <div className={`role-option ${role==="partner"?"selected":""}`} onClick={()=>setRole("partner")}>
            <h3>💚 I'm the Partner / Caregiver</h3><p>Link to a patient's profile to see protocols, log your daily exposure, and stay coordinated.</p>
          </div>
        ...
        {step==="setup"&&<>
          ...
          <p>Enter your first name. You'll get a code to share with your partner so they can link to your dashboard.</p>
          <div className="form-group" style={{marginBottom:16}}>
            <label className="form-label">First Name</label>
            <input className="form-input" placeholder="e.g. Alex" value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&createPatient()}/>
          </div>
          <button className="btn btn-primary" style={{width:"100%",padding:12}} disabled={!name.trim()||busy} onClick={createPatient}>{busy?"Creating...":"Create Profile"}</button>
          <button className="btn btn-ghost" style={{width:"100%",marginTop:8,padding:10}} onClick={()=>setStep("role")}>Back</button>
        </>}
        {step==="done"&&<>
          <h1>You're Set Up</h1>
          ...
          <p style={{fontSize:12,color:"var(--text-muted)",textAlign:"center"}}>Your partner will enter this code when they set up their account.</p>
        ...
        {step==="link"&&<>
          ...
          <p>Enter the share code from your patient's setup, along with your name.</p>
          <div className="form-group" style={{marginBottom:12}}>
            <label className="form-label">Your First Name</label>
            <input className="form-input" placeholder="e.g. Jordan" value={name} onChange={e=>setName(e.target.value)}/>
          </div>
          <div className="form-group" style={{marginBottom:16}}>
          ...
          <button className="btn btn-primary" style={{width:"100%",padding:12,marginTop:8}} disabled={code.length<4||busy} onClick={linkPartner}>{busy?"Linking...":"Link Account"}</button>
          <button className="btn btn-ghost" style={{width:"100%",marginTop:8,padding:10}} onClick={()=>setStep("role")}>Back</button>
        </>}
      </div>
    </div>
...
export default function TransplantCare() {
    ...
    (async () => {
      ...
      if (saved) {
        ...
        if (saved.role === "patient") {
          const { name, code, role, createdAt, ...data } = saved;
          setPd(prev => ({ ...prev, ...data }));
        } else {
          setView("partner");
          const linked = await shGet(`tc-patient:${saved.linkedCode}`);
          if (linked) { const { name, code, role, createdAt, ...data } = linked; setPd(prev => ({ ...prev, ...data })); }
  ...
  async function logout() {
    ...
    setProfile(null); setPd(DEFAULT_PATIENT); setAssessments([]); setExpLevel("GREEN"); setView("patient");
  }

  const phase = getPhase(pd.alc);
  const iS = calcInfectionScore(pd.tacroTrough, pd.mmfDose, pd.predDose);
  const rS = calcRejectionScore(pd.tacroTrough, pd.mmfDose, pd.predDose);
  const cS = Math.round((iS + rS) / 2);
  const iM = getImmunoMessage(iS, rS);
  const sS = getSymptomStatus(pd.temp, pd.systolic, pd.diastolic, pd.urineOutput, pd.cough, pd.gi, pd.fatigue);
  ...
  if (!profile) return <><style>{CSS}</style><Onboarding onComplete={p=>{setProfile(p);if(p.role==="partner")setView("partner");}}/><button className="about-toggle" onClick={()=>setShowAbout(true)} title="About this project">?</button>{showAbout&&<AboutPanel onClose={()=>setShowAbout(false)}/>}</>;
      ...
      <div className="app-container">
        <header className="header">
          ...
          <div className="header-right">
            <div className="phase-badge" style={{background:phase.bg,color:phase.color}}>
              <span style={{width:8,height:8,borderRadius:"50%",background:phase.color,display:"inline-block"}}/>{phase.name} · {phase.label}
            </div>
            <div className="user-pill">
              {isPat?"🏥":"💚"} {profile.name||(isPat?"Patient":"Partner")}
        ...
        {view==="patient"&&<div className="grid">
          <div className="card">
            ...
            <div className="phase-display"><span className="phase-name" style={{color:phase.color}}>{phase.name}</span><div className="phase-detail"><strong>ALC: {pd.alc.toFixed(2)} K/µL</strong><br/>Week {pd.weeksPostATG} post-ATG</div></div>
            {isPat&&<div className="form-row">
              <div className="form-group"><label className="form-label">Latest ALC (K/µL)</label><input className="form-input" type="number" step="0.01" min="0" value={pd.alc} onChange={e=>up({alc:parseFloat(e.target.value)||0})}/></div>
          ...
          <div className="card">
            ...
            <div style={{display:"flex",gap:16,flexWrap:"wrap",marginTop:8}}>
              {PHASES.map(p=><span key={p.name} style={{fontSize:11,color:p.color,display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:"50%",background:p.color,display:"inline-block"}}/>{p.name}: ≥{p.alcMin}</span>)}
            </div>
          </div>
          ...
          <div className="card">
            <div className="card-header"><div className="card-title">Community Risk</div><span className="status-pill" style={{background:"var(--surface-alt)",color:"var(--text-secondary)"}}>LA County</span></div>
            <p style={{fontSize:13,color:"var(--text-secondary)",lineHeight:1.5,margin:"8px 0 16px"}}>Current respiratory illness data for Los Angeles County including COVID-19, Influenza, and RSV trends.</p>
            <a href="http://publichealth.lacounty.gov/acd/respwatch.htm" target="_blank" rel="noopener noreferrer" className="community-link">Open LA County Respiratory Watch <span style={{fontSize:14}}>↗</span></a>
          </div>
          ...
          <div className="card">
            <div className="card-header"><div><div className="card-title">Immunosuppression Monitor</div><div className="card-subtitle">Last updated: {lu}</div></div>
              <span className="status-pill" style={{background:cS<=3?"var(--green-bg)":cS<=6?"var(--yellow-bg)":"var(--red-bg)",color:cS<=3?"var(--green)":cS<=6?"var(--yellow)":"var(--red)"}}>Score: {cS}/10</span>
            </div>
          ...
          <div className="card">
            ...
            <div className="form-group" style={{marginBottom:12}}><label className="form-label">Temperature (°F)</label><input className="form-input" type="number" step="0.1" value={pd.temp} disabled={!isPat} onChange={e=>up({temp:parseFloat(e.target.value)||0})}/></div>
            ...
            <div className="form-group" style={{marginBottom:0}}><label className="form-label">Urine Output</label><select className="symptom-select" value={pd.urineOutput} disabled={!isPat} onChange={e=>up({urineOutput:e.target.value})}><option>Normal</option><option>Low</option><option>Very Low</option></select></div>
            <div className="symptom-grid">
              {[["Cough","cough"],["GI Issues","gi"],["Fatigue","fatigue"]].map(([l,k])=>
                <div key={k} className="form-group"><label className="form-label">{l}</label><select className="symptom-select" value={pd[k]} disabled={!isPat} onChange={e=>up({[k]:e.target.value})}><option>None</option><option>Mild</option><option>Moderate</option><option>Severe</option></select></div>
              )}
            </div>
            {sS.label==="Concerning"&&<div className="symptom-alert" style={{background:"var(--red-bg)",color:"var(--red)"}}>⚠️ 2+ abnormal parameters — Seek Evaluation</div>}
            {sS.label==="Monitor"&&<div className="symptom-alert" style={{background:"var(--yellow-bg)",color:"var(--yellow)"}}>⚡ Monitor closely — contact team if worsening</div>}
          </div>
        </div>}
        ...
        {view==="partner"&&<div className="grid">
          ...
          <div className="card">
            ...
            <p style={{fontSize:13,color:"var(--text-secondary)",marginBottom:12}}>Select today's highest exposure activity:</p>
            {[["GREEN","green","🟢 LOW — Green","Home / solo errands, brief masked indoor stops"],["YELLOW","yellow","🟡 MODERATE — Yellow","Office work, indoor dining, small gatherings"],["RED","red","🔴 HIGH — Red","Crowded events, flights, parties, concerts"]].map(([k,c,t,d])=>
          ...
          <div className="card grid-full">
            <div className="card-header"><div className="card-title">Patient Current Status</div><span className="status-pill" style={{background:phase.bg,color:phase.color}}>{phase.name} · {phase.label}</span></div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(180px, 1fr))",gap:16}}>
              <div><div className="form-label">ALC Level</div><div style={{fontSize:20,fontWeight:600,marginTop:4}}>{pd.alc.toFixed(2)} <span style={{fontSize:13,color:"var(--text-muted)"}}>K/µL</span></div></div>
              ...
              <div><div className="form-label">Immuno Balance</div><span className="status-pill" style={{background:cS<=3?"var(--green-bg)":cS<=6?"var(--yellow-bg)":"var(--red-bg)",color:cS<=3?"var(--green)":cS<=6?"var(--yellow)":"var(--red)",marginTop:4,display:"inline-block"}}>{cS}/10</span></div>
            </div>
            <div className="divider"/>
